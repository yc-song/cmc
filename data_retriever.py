...
class ZeshelDataset(MentionSet):
    def __init__(self, tokenizer, mentions, doc, max_len,
                 ...
                 all_entity_token_ids, all_entity_masks, args, retriever = False):
        ...
        self.max_len = int(args.L/2)
        self.full_max_len = args.L
    ...
    def __getitem__(self, index):
        ...
        mention_token_ids = torch.tensor(mention_token_ids).long()
        mention_masks = torch.tensor(mention_masks).long()
        candidate_token_ids = torch.tensor(np.array(candidate_token_ids)).long()
        candidate_masks = torch.tensor(np.array(candidate_masks)).long()
        if self.type_model == "full" and self.retriever == False:
            encoded_pairs = torch.zeros((self.num_cands+1, self.full_max_len))
            type_marks = torch.zeros((self.num_cands+1, self.full_max_len))
            input_lens = torch.zeros(self.num_cands+1)
            for i, candidate_document_id in enumerate(candidate_token_ids):
                input_lens[i] = int(len(mention_window) + 1 + candidate_document_id.nonzero().size(0))
                encoded_token_ids = torch.cat([mention_token_ids[:len(mention_window)+2], candidate_document_id[1:], torch.zeros(int(self.full_max_len- mention_token_ids[:len(mention_window)+2].size(0)-candidate_document_id[1:].size(0)))])
                encoded_pairs[i] = torch.tensor(encoded_token_ids)
                mask = torch.zeros_like(encoded_token_ids)
                mask[len(mention_window)+2:int(input_lens[i].item())] = 1
                # Use torch.where to fill the values based on the mask
                type_marks[i] = mask
    ...
    def get_candidate_prefix(self, candidate_document_id):
        ...
        tokens = self.all_entity_token_ids[candidate_document_id]['text'].split()
...
def load_zeshel_data(train_set, macro_eval=True):
    """
    train set: mention_token_ids, mention_masks, candidate_token_ids, candidate_masks
