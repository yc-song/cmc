...
def main(args):
    ...
    if training_finished:
        cpt = torch.load(os.path.join(args.save_dir,"epoch_16.bin")) if device.type == 'cuda' \
                else torch.load(os.path.join(args.save_dir,"epoch_16.bin") , map_location=torch.device('cpu'))
        ...
        print('start evaluation on val set (to verify whether correct model loaded)')
        args.val_random_shuffle = False
        loader_train, loader_val, loader_test, \
        num_val_samples, num_test_samples = get_loaders(data, tokenizer, args.L,
                                                    args.C, args.B,
                                                    args.num_workers,
